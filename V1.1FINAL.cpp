...
int main()
{
	...
	cout << "Nuskaitymas is failo bei studentu suskirstymas i kietus ir vargsus vektoriuose - spauskite F." << endl;
	cout << "Duomenu vedimas ranka - spauskite R." << endl;
	cin >> simbolis;
	if (simbolis != "F" && simbolis != "f" && simbolis != "R" && simbolis != "r") {
		...
		Ivedimas(st);
		cout << "Jei norite studentu galutini pazymi gauti pagal vidurki - spauskite V.\n";
		cout << "Jei norite studentu galutini pazymi gauti pagal mediana - spauskite M.\n";
		cin >> ivesk;
		if (ivesk != "V" && ivesk != "v" && ivesk != "M" && ivesk != "m") {
			cout << "blogas pasirinkimas" << endl;
			exit(1);
		}
		else if (ivesk == "V" || ivesk == "v") {
			Skaiciavimai(st);
...
bool payginimas(const studentas& x, const studentas& y)
{
